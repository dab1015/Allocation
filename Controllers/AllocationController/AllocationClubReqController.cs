...
namespace SNRWMSPortal.Controllers
{
    ...
    public class AllocationClubReqController : Controller
    {
        ...
        public ActionResult ApprovedRequest(List<AllocationClub> allocationClubModels)
        {
            ...
            try
            {
                ...
                foreach (var item in allocationClubModels)
                {
                    ...
                    {
                        queryskus.UpdateApprovedAllocation(skucode, clubcode,quantity,reasonname, todaysDate);
                    }
                    else if(insertAllocbool == false)
                    {
                        queryskus.InsertAllocation(skucode, clubcode, quantity, 5,5, dconfig, reasonname, dcmoh, todaysDate);
                    }
