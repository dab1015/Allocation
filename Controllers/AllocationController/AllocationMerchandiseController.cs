...
namespace SNRWMSPortal.Controllers
{
   [AuthorizeRoles(Role.Allocation, Role.SystemAdministrator)]
    public class AllocationMerchandiseController : Controller
    {
            ...
            try
            {
                ...
                foreach (var item in allocationOverModels)
                {
                    ...
                    if (insertDeptbool == true)
                    {
                        ...
                        queryskus.UpdateMerchan(skucode,clubcode,totals,reason,"MERCHANDISE",userName,todaysDate,dconfig,description);
                    ...
                    else
                    {
                        ...
                        queryskus.InsertMerchan(skucode,clubcode,totals,reason,"MERCHANDISE",userName,todaysDate,dconfig,description);
                        
                    }
                    if (insertDeptbool1 == true)
                    {
                        
                        queryskus.UpdateMerchanAllocation(skucode, clubcode, totals,reasonname,4,dcmoh, todaysDate);
                        queryskus.DeleteMerchanAllocation(skucode, clubcode, 4);
                    }
